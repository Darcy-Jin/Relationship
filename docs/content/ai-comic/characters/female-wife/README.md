...
- `profile_subject_left` 已完成最终 Human Gate；
- 用户最终选择 A，并明确“不再改了，就是这版”；
- 该结果已升级为 `MASTER-SUP-06` / Support Master；
- 早期 SIDE-CANDIDATE / SIDE-DIAG 继续保留为历史诊断证据，不再代表当前状态。
...
| Profile / subject_left | Validated | High |

关键原则：

> **正脸不能唯一推导侧脸；但所有已验证视图必须共同约束同一个 Cross-view Identity Model。**

`profile_subject_left` 已通过 Human Recognition + Cross-view Structural Consistency，并升级为 `MASTER-SUP-06`。

其他未验证方向 / 视角仍按 Coverage 驱动，不因为这一侧已经通过就自动视为全部 profile 已验证。
