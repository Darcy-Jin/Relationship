...
- 老婆脸版正式入口：`docs/content/ai-comic/characters/female-wife-face-v0.md`，当前版本 **v0.8**。
- 当前状态是 **Identity Anchor Draft + Positive Sample / Near Pass**：用户已明确认为最新校准板“整体很像”，但尚未冻结为 Identity Master。
- 当前正式把两层分开：
  - **Identity｜她是谁**：以前照片 + 现在照片共同提炼跨年龄稳定特征；
  - **Current State｜现在的她**：由现在照片决定当前年龄感、双眼皮、胖瘦、软组织、发型、眼镜、皮肤与精神状态。
- 当前已较稳定的 Identity 特征：
  - 头脸包络小而紧凑、偏窄长的椭圆感；
  - 五官相对集中；
  - 头顶圆润但不能画成大头；
  - 颧骨与苹果肌有真实体积；
  - 下颌柔和，静态下巴圆润；
  - 下颊 / 下巴两侧保留少量软组织与轻微婴儿肥；
  - 眼神软、松、略朦胧，不是亮、锐、闪的“有神眼”；
  - 习惯性微笑有“努嘴角 / 略僵 / 古灵精怪”的本人特点；真正开心时才明显全脸一起笑。
- 当前粗测只作为生成约束：头顶到下巴 / 最大头脸宽约 **1.35～1.40**；眼线到嘴线的主要五官区约占整个头高 **30～32%**。多人合照只能弱支持“绝对头小”直觉，不作为真实头围测量。
- 下一轮只做：**Identity 文字约束 + 少量现在照片 → 验证 Current State 下是否仍然像本人**。通过后再冻结 **Female Identity Master v1 + Current State v1**，之后才进入多画风。
...
保留已确认的普通女性角色基线
↓
老婆脸版：跨年龄照片共同提炼 Identity
↓
保留当前 Positive Sample 的有效身份特征
↓
只用少量现在照片定义 Current State
↓
验证“同一 Identity 在现在状态下是否仍然像”
↓
通过后冻结 Female Identity Master v1 + Current State v1
↓
