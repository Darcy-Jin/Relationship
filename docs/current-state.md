...
- 老婆脸版正式入口：`docs/content/ai-comic/characters/female-wife-face-v0.md`，当前版本 **v0.10**。
- 当前阶段正式收敛为 **Current Identity Calibration｜当前身份校准**：这一轮只参照最开始上传、头发大多扎起来的近期照片，先把“现在的她”画准。
- 以前的披发 / 短发年轻照片继续保留，但只用于历史身份参考和结果漂移时纠偏，不直接主导当前生成。
- 当前身份标准：**小而紧凑、偏窄长的椭圆头脸 + 五官相对集中 + 头顶圆润 + 颧骨 / 苹果肌有体积 + 柔和下颌 + 圆下巴 + 下巴两侧 / 下颊少量软组织 + 软、松、略朦胧的眼神 + 略僵、努嘴角、古灵精怪的习惯性微笑**。
- 当前验收优先级：**像本人 > 头脸结构 > 五官位置 > 眼神 / 表情 > 胖瘦状态 > 好看 > 画风**。
- 胖瘦、苹果肌饱满度、下颊软组织多少全部后置：只有“这张脸基本就是她”以后才做微调。
- 当前已有一版用户认为“整体很像”的 Positive Sample，继续保留其有效身份特征；但当前不直接复用其中年轻状态，而要验证这些 Identity 特征能否稳定落到现在状态。
- 下一轮：**现在原图 vs 当前绘制**，只画正脸自然、习惯性微笑 / 努嘴角、轻 3/4。通过后先做胖瘦 / 软组织微调，再冻结 **Female Current Identity Master v1**，之后才进入多画风。
...
保留普通女性角色基线
↓
老婆脸版只用现在照片做 Current Identity Calibration
↓
先锁头脸结构 / 五官位置 / 眼神 / 表情
↓
用户确认“基本就是她”
